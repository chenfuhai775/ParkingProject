...
namespace Parking.DBService.BLL
{
	...
	/// 需要发送的报文临时存
	/// </summary>
    public partial class WF_ProcessNode : IWF_ProcessNode
	{
		private readonly Parking.DBService.DAL.WF_ProcessNode dal=new Parking.DBService.DAL.WF_ProcessNode();
		...
		/// 获得数据列表
		/// </summary>
		public List<Parking.Core.Model.WF_ProcessNode> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Parking.Core.Model.WF_ProcessNode> DataTableToList(DataTable dt)
		{
			List<Parking.Core.Model.WF_ProcessNode> modelList = new List<Parking.Core.Model.WF_ProcessNode>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Parking.Core.Model.WF_ProcessNode model;
