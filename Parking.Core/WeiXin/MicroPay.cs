...
namespace Parking.Core.WeiXin
{
    public class MicroPay
    {
        ...
        */
        ...
        {
            LogHelper.Log.Info("Micropay is processing...");
            ...
            if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
            {
                ...
                LogHelper.Log.Error("Micropay API interface call failure, result : " + result.ToXml());
            }

            //签名验证
            result.CheckSign();
            LogHelper.Log.Debug("Micropay response check sign success");

            //刷卡支付直接成功
            if (result.GetValue("return_code").ToString() == "SUCCESS" &&
                result.GetValue("result_code").ToString() == "SUCCESS")
            {
                LogHelper.Log.Debug("Micropay business success, result : " + result.ToXml());
            ...
            {
                LogHelper.Log.Error("micropay API interface call success, business failure, result : " + result.ToXml());
                return result.ToPrintStr();
            }
            ...
            while (queryTimes-- > 0)
            {
                ...
                {
                    LogHelper.Log.Debug("Mircopay success, return order query result : " + queryResult.ToXml());
                    return queryResult.ToPrintStr();
                }
                //订单交易失败，直接返回刷卡支付接口返回的结果，失败原因会在err_code中描述
                else
                {
                    LogHelper.Log.Error("Micropay failure, return micropay result : " + result.ToXml());
                    return result.ToPrintStr();
                }
            }

            //确认失败，则撤销订单
            LogHelper.Log.Error("Micropay failure, Reverse order is processing...");
            if (!Cancel(out_trade_no))
            {
                LogHelper.Log.Error("Reverse order failure");
            }
